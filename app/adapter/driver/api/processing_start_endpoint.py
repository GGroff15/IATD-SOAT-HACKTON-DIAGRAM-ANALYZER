...
from app.infrastructure.logging.correlation import clear_correlation_id, set_correlation_id
...
def _parse_upload_from_s3_uri(file_url: str, mimetype: str, protocol: str) -> DiagramUpload:
    ...
    uri_without_scheme = file_url[len("s3://") :]
    ...
    if folder in {"", "."}:
        raise ValueError("file.url must include a folder path")
    ...
    return DiagramUpload(diagram_upload_id=diagram_uuid, folder=folder, extension=extension)
